...
UInt32	BGM_PlugIn::GetPropertyDataSize(AudioObjectID inObjectID, pid_t inClientPID, const AudioObjectPropertyAddress& inAddress, UInt32 inQualifierDataSize, const void* inQualifierData) const
{
	...
	switch(inAddress.mSelector)
	{
		...
		case kAudioPlugInPropertyDeviceList:
            ...
            DebugMsg("[DEBUG]Plug In 1");
            theAnswer = /*(BGM_NullDevice::GetInstance().IsActive() ? 3 : 2)*/1 * sizeof(AudioObjectID);
			break;
		...
		case kAudioPlugInPropertyDeviceList:
            {
                ...
                DebugMsg("[DEBUG] Get Property Data default 1");
...
//            break;
		...
		default:
            DebugMsg("[DEBUG] Set Property Data");
